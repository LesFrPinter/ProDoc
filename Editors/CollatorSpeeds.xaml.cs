...
namespace ProDocEstimate.Editors
{
    public partial class CollatorSpeeds : Window, INotifyPropertyChanged
    {
        ...
        public CollatorSpeeds()
        {
            InitializeComponent();
            string cmd = "SELECT * FROM [ESTIMATING].[dbo].[Collator_Speeds] ORDER BY ID";

            SqlConnection conn = new SqlConnection(ConnectionString);
            SqlDataAdapter da = new SqlDataAdapter(cmd, conn);
            dt = new DataTable("Coll");
            da.Fill(dt);
            dv = dt.DefaultView;
        ...
        private void grdData_CellEditEnding(object sender, System.Windows.Controls.DataGridCellEditEndingEventArgs e)
        {
            ...
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Collator_Speeds] set {colName} =  {editedCellValue}  WHERE ID = '{ID}'"; }
            else
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Collator_Speeds] set {colName} = '{editedCellValue}' WHERE ID = '{ID}'"; };
