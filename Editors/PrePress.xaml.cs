...
namespace ProDocEstimate.Editors
{
    public partial class PrePress : Window, INotifyPropertyChanged
    {
        ...
        {
            InitializeComponent();
            string cmd = "SELECT * FROM [ESTIMATING].[dbo].[PrePressOESpeeds] ORDER BY ID";

            SqlConnection conn = new SqlConnection(ConnectionString);
            SqlDataAdapter da = new SqlDataAdapter(cmd, conn);
            dt = new DataTable("PrePress");
            da.Fill(dt);
            dv = dt.DefaultView;
        ...
        private void grdData_CellEditEnding(object sender, System.Windows.Controls.DataGridCellEditEndingEventArgs e)
        {
            ...
            { cmd = $"UPDATE [ESTIMATING].[dbo].[PrePressOESpeeds] set {colName} =  {editedCellValue}  WHERE ID = '{ID}'"; }
            else
            { cmd = $"UPDATE [ESTIMATING].[dbo].[PrePressOESpeeds] set {colName} = '{editedCellValue}' WHERE ID = '{ID}'"; };
