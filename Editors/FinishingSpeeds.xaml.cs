...
namespace ProDocEstimate.Editors
{
    public partial class FinishingSpeeds : Window, INotifyPropertyChanged
    {
        ...
        public FinishingSpeeds()
        {
            InitializeComponent();
            string cmd = "SELECT * FROM [ESTIMATING].[dbo].[Finishing_Speeds] ORDER BY ID";
    ...
    private void mnuFileExit_Click(object sender, RoutedEventArgs e)
        ...
        private void grdData_CellEditEnding(object sender, System.Windows.Controls.DataGridCellEditEndingEventArgs e)
        {
            ...
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Finishing_Speeds] set {colName} =  {editedCellValue}  WHERE ID = '{ID}'"; }
            else
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Finishing_Speeds] set {colName} = '{editedCellValue}' WHERE ID = '{ID}'"; };
