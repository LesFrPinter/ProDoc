...
{
    public partial class InkCost : Window, INotifyPropertyChanged
    {
        #region Properties
        ...
        {
            InitializeComponent();
            string cmd = "SELECT * FROM [ESTIMATING].[dbo].[Ink_Cost] ORDER BY ID";
        ...
        private void grdData_CellEditEnding(object sender, System.Windows.Controls.DataGridCellEditEndingEventArgs e)
        {
            ...
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Ink_Cost] set {colName} =  {editedCellValue}  WHERE ID = '{ID}'"; }
            else
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Ink_Cost] set {colName} = '{editedCellValue}' WHERE ID = '{ID}'"; };
