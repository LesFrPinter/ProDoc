...
{
    public partial class PressSpeeds : Window, INotifyPropertyChanged
    {
        #region Properties
        ...
        {
            InitializeComponent();
            string cmd = "SELECT * FROM [ESTIMATING].[dbo].[Press_Speeds] ORDER BY ID";
        ...
        private void grdData_CellEditEnding(object sender, System.Windows.Controls.DataGridCellEditEndingEventArgs e)
        {
            ...
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Press_Speeds] set {colName} =  {editedCellValue}  WHERE ID = '{ID}'"; }
            else
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Press_Speeds] set {colName} = '{editedCellValue}' WHERE ID = '{ID}'"; };
