...
{
    public partial class Quotes : Window, INotifyPropertyChanged
    {
        #region Properties
        ...
        {
            InitializeComponent();
            string cmd = "SELECT * FROM [ESTIMATING].[dbo].[Quotes] ORDER BY ID";
        ...
        private void grdData_CellEditEnding(object sender, System.Windows.Controls.DataGridCellEditEndingEventArgs e)
        {
            ...
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Quotes] set {colName} =  {editedCellValue}  WHERE ID = '{ID}'"; }
            else
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Quotes] set {colName} = '{editedCellValue}' WHERE ID = '{ID}'"; };
