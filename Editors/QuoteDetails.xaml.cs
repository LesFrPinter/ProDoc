...
{
    public partial class QuoteDetails : Window, INotifyPropertyChanged
    {
        #region Properties
        ...
        {
            InitializeComponent();
            string cmd = "SELECT * FROM [ESTIMATING].[dbo].[Quote_Details] ORDER BY ID";
        ...
        private void grdData_CellEditEnding(object sender, System.Windows.Controls.DataGridCellEditEndingEventArgs e)
        {
            ...
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Quote_Details] set {colName} =  {editedCellValue}  WHERE ID = '{ID}'"; }
            else
            { cmd = $"UPDATE [ESTIMATING].[dbo].[Quote_Details] set {colName} = '{editedCellValue}' WHERE ID = '{ID}'"; };
